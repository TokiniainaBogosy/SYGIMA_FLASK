  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/v1/materiel/stock/update/${selectedStock.id}`, {
        ...
        body: JSON.stringify({ designation: newDesignation, quantite_actuelle: newUnite }),
      });

      if (response.ok) {
        setSelectedStock(null); // Fermer le modal
        setStocks(prev => prev.map(s => s.id === selectedStock.id ? { ...s, designation: newDesignation, unite: newUnite } : s)); // Rafraîchir la liste
      }
    } catch (error) {
